...
export const CorporateFiltersProvider: React.FC<{
  ...
  // Actions - todas memoizadas para evitar re-renders
    ...
    (data: any[]) => {
      ...
      return data.filter((item) => {
        ...
        if (state.processType && state.processType !== '') {
          // Mapeia os valores do select para os valores reais
        ...
        if (state.urgency && state.urgency !== '') {
          // Calcula a urgência baseada na data de expiração
          const calculateUrgency = (expireDate: Date) => {
            ...
            if (diffDays < 0) return 'critical'; // Expirado
            if (diffDays <= 7) return 'urgent'; // Menos de 7 dias
            if (diffDays <= 15) return 'warning'; // Menos de 15 dias
  ...
  const getActiveFiltersLabels = useCallback(() => {
    ...
    if (state.accounting) {
      labels.push({
        key: 'accounting',
        label: 'Contabilidade',
        value: state.accounting, // Idealmente buscar o nome pelo ID
      });
    }
...
// Hook
export const useCorporateFilters = (): CorporateFiltersContextType => {
