export function extractErrorDetails(errors: any): Array<{field: string, message: string}> {
  ...
  export function validateResponsibilityFields(currentValues: any, form: any, setShowFieldErrors: (show: boolean) => void) {
    ...
    if (!currentValues.info_adicionais.area_resp || currentValues.info_adicionais.area_resp <= 0) {
