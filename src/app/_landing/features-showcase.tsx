...
function ClientNotificationsAnimation() {
  ...
  const emails = [
    { subject: "Viabilidade Aprovada", preview: "Sua empresa ABC Ltda avançou para a próxima etapa" },
    { subject: "Contrato Social Pronto", preview: "O documento foi elaborado e está disponível para revisão" },
    { subject: "Registro na Junta", preview: "Iniciamos o processo de registro na Junta Comercial" },
  ];
  ...
  useEffect(() => {
    ...
    const runAnimation = () => {
      ...
      timers.push(setTimeout(() => { setEmailOpen(false); setCurrentEmail(2); }, 2800));
      timers.push(setTimeout(() => setEmailOpen(true), 3400));
      timers.push(setTimeout(() => { setEmailOpen(false); setCurrentEmail(3); }, 5000));
    ...
    <div className="flex h-full w-full flex-col gap-2 p-3 overflow-hidden">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className={cn(
            ...
            <User className={cn("h-4 w-4 transition-colors duration-300", showHappy ? "text-primary" : "text-foreground")} />
          </div>
          <div className="flex flex-col">
            <span className="text-[10px] font-semibold">Cliente</span>
            {currentEmail > 0 && !showHappy && (
              <span className="text-[9px] text-blue-500 animate-in fade-in duration-200">{currentEmail} email(s) recebido(s)</span>
            )}
            {showHappy && (
              <span className="text-[9px] text-primary animate-in fade-in duration-200">Informado e tranquilo</span>
            )}
          </div>
        </div>
        {showHappy ? (
          <div className="flex items-center gap-1 rounded-full bg-primary/10 px-2 py-0.5 animate-in zoom-in duration-300">
            ...
            <span className="text-[9px] font-medium text-primary">Satisfeito</span>
          </div>
        ) : (
      ...
      <div className="flex-1 flex flex-col gap-1 overflow-hidden">
        {emails.slice(0, currentEmail).map((email, i) => (
          ...
          >
            <Mail className={cn("h-3 w-3 shrink-0", i === currentEmail - 1 && emailOpen ? "text-blue-500" : "text-muted-foreground")} />
            <div className="min-w-0 flex-1">
              <p className="text-[9px] font-semibold truncate">{email.subject}</p>
              <p className="text-[8px] text-muted-foreground truncate">{email.preview}</p>
            </div>
            <CheckCircle2 className={cn("h-3 w-3 shrink-0 transition-colors duration-300", i === currentEmail - 1 && emailOpen ? "text-blue-500" : "text-muted-foreground/40")} />
          </div>
        ))}
        {/* Placeholder rows */}
        {Array.from({ length: Math.max(0, 3 - currentEmail) }).map((_, i) => (
          <div key={`ph-${i}`} className="flex items-center gap-2 rounded-lg border border-dashed bg-muted/20 p-1.5 opacity-30">
      ...
      <div className={cn(
        ...
        {showHappy ? (
          <>
            <Heart className="h-3 w-3 text-primary fill-primary" />
            <span className="text-[9px] font-medium text-primary">Cliente informado e tranquilo</span>
          </>
        ) : (
          <>
            <Mail className="h-3 w-3 text-muted-foreground" />
            <span className="text-[9px] text-muted-foreground">Recebendo atualizações por email</span>
          </>
        )}
      </div>
    </div>
...
function ContadorAlertsAnimation() {
  ...
  useEffect(() => {
    ...
    const runAnimation = () => {
      ...
      timers.push(setTimeout(() => { setStalledDays(3); setShowAlert(true); }, 1800));
    ...
    <div className="flex h-full w-full flex-col gap-2 p-3 overflow-hidden">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className={cn(
          ...
          <Clock className={cn("h-3 w-3", stalledDays >= 3 ? "text-amber-500" : "text-muted-foreground")} />
          <span className={cn("text-[9px] font-medium tabular-nums", stalledDays >= 3 ? "text-amber-500" : "text-muted-foreground")}>
            {stalledDays} dias
          </span>
        </div>
      </div>
      ...
      <div className={cn(
        ...
        <div className="flex items-center gap-2 mb-2">
          ...
          <div>
            ...
            <p className="text-[9px] text-muted-foreground">Viabilidade · 2/5 etapas</p>
          </div>
        </div>
        ...
        <div className={cn(
          ...
          {stalledDays >= 3 ? (
            <>
              ...
              <span className="text-[9px] text-amber-600 font-medium">Processo parado há {stalledDays} dias</span>
            </>
          ) : (
            <>
              <Clock className="h-3 w-3 text-muted-foreground shrink-0" />
              <span className="text-[9px] text-muted-foreground">Monitorando...</span>
            </>
          )}
        </div>
      </div>
      ...
      {showAlert && (
        <div className="rounded-xl border-2 border-amber-500 bg-background overflow-hidden animate-in slide-in-from-top-2 fade-in duration-300">
          <div className="flex items-center gap-2 bg-amber-500 px-2.5 py-1.5">
            ...
            <span className="text-[9px] font-medium text-white">Alerta para Contador</span>
          </div>
          <div className="p-2 space-y-1.5">
            <p className="text-[9px] leading-relaxed">
              <span className="font-semibold">Empresa XYZ</span> parada há{" "}
              <span className="font-semibold text-amber-600">3 dias</span> na etapa de Viabilidade.
            </p>
            {showSuggestion && (
      ...
      <div className={cn(
        ...
        showAlert ? "bg-amber-500/10 border border-amber-500/20" : "bg-muted/50"
      )}>
        {showAlert ? (
          <>
            <AlertTriangle className="h-3 w-3 text-amber-500" />
            <span className="text-[9px] font-medium text-amber-600">Contador notificado</span>
          </>
        ) : (
          <>
            <div className="h-2 w-2 rounded-full bg-primary animate-pulse" />
            <span className="text-[9px] text-muted-foreground">Sistema monitorando processos</span>
          </>
        )}
      </div>
    </div>
...
function EmailFlowAnimation() {
  const [emails, setEmails] = useState<{ to: string; subject: string; sent: boolean }[]>([]);
  ...
  useEffect(() => {
    ...
    const runAnimation = () => {
      ...
      emailQueue.forEach((email, i) => {
        ...
        timers.push(setTimeout(() => {
          setEmails(prev => prev.map((e, idx) => idx === i ? { ...e, sent: true } : e));
        }, 600 + i * 1200 + 600));
      });
      timers.push(setTimeout(runAnimation, 600 + emailQueue.length * 1200 + 1500));
    };
    ...
    <div className="relative flex h-full w-full flex-col p-4 overflow-hidden">
      <div className="flex items-center gap-2 mb-3 pb-2 border-b">
        ...
        <div>
          ...
          <p className="text-[9px] text-muted-foreground">Disparados por eventos</p>
        </div>
      ...
      <div className="flex-1 space-y-2 overflow-hidden">
        {emails.map((email, i) => (
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <p className="text-[10px] font-medium truncate">{email.subject}</p>
              <p className="text-[9px] text-muted-foreground truncate">{email.to}</p>
            </div>
            {email.sent && <span className="text-[8px] text-primary font-medium shrink-0">Enviado</span>}
          </div>
        ))}
        ...
        {emails.length < 3 && (
          <div className="space-y-2">
            {Array.from({ length: 3 - emails.length }).map((_, i) => (
              <div key={`placeholder-${i}`} className="flex items-center gap-2 rounded-lg border border-dashed bg-muted/20 p-2 opacity-40">
      ...
      <div className="mt-3 pt-2 border-t flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          <div className="h-2 w-2 rounded-full bg-primary animate-pulse" />
          <span className="text-[9px] text-muted-foreground">Monitorando eventos</span>
        </div>
        <span className="text-[10px] font-medium text-primary">{emails.filter(e => e.sent).length}/3 enviados</span>
      </div>
    </div>
  );
}
    ...
    <section id="como-funciona" className="py-20 md:py-28">
      <div className="mx-auto mb-14 max-w-2xl space-y-3 text-center">
        ...
        <p className="text-muted-foreground text-sm leading-relaxed md:text-base">
          Cada movimentação gera uma notificação. O cliente sabe exatamente o que está acontecendo,
          e o contador nunca deixa um processo parar. Resultado: empresas abertas mais rápido.
        </p>
      </div>
