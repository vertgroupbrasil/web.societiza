...
const customErrorMap: z.ZodErrorMap = (issue, ctx) => {
  switch (issue.code) {
    ...
    case z.ZodIssueCode.invalid_literal:
      return {
        message: `Valor literal inválido. Esperado ${JSON.stringify(issue.expected)}`,
      };
    ...
    case z.ZodIssueCode.too_small:
      if (issue.type === 'array') {
        return {
          message: `Array deve conter ${issue.exact ? 'exatamente' : 'pelo menos'} ${issue.minimum} elemento(s)`,
        };
      }
      if (issue.type === 'string') {
        return {
          message: `String deve conter ${issue.exact ? 'exatamente' : 'pelo menos'} ${issue.minimum} caractere(s)`,
        };
      }
      if (issue.type === 'number') {
        return {
          message: `Número deve ser ${issue.exact ? 'exatamente' : 'maior ou igual a'} ${issue.minimum}`,
        };
      }
      if (issue.type === 'date') {
        return {
          message: `Data deve ser ${issue.exact ? 'exatamente' : 'maior ou igual a'} ${new Date(Number(issue.minimum))}`,
        };
      }
    ...
    case z.ZodIssueCode.too_big:
      if (issue.type === 'array') {
        return {
          message: `Array deve conter ${issue.exact ? 'exatamente' : 'no máximo'} ${issue.maximum} elemento(s)`,
        };
      }
      if (issue.type === 'string') {
        return {
          message: `String deve conter ${issue.exact ? 'exatamente' : 'no máximo'} ${issue.maximum} caractere(s)`,
        };
      }
      if (issue.type === 'number') {
        return {
          message: `Número deve ser ${issue.exact ? 'exatamente' : 'menor ou igual a'} ${issue.maximum}`,
        };
      }
      if (issue.type === 'bigint') {
        return {
          message: `BigInt deve ser ${issue.exact ? 'exatamente' : 'menor ou igual a'} ${issue.maximum}`,
        };
      }
      if (issue.type === 'date') {
        return {
          message: `Data deve ser ${issue.exact ? 'exatamente' : 'menor ou igual a'} ${new Date(Number(issue.maximum))}`,
        };
      }
...
export const portugueseMessages = {
  ...
  password:
    'A senha deve ter pelo menos 8 caracteres, incluindo pelo menos uma letra maiúscula, uma minúscula e um número',
};
